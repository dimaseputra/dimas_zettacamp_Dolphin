...
function purchasing(book, purchasedAmount, creditPerMonth) {
  ...
  let creditPrice = totalPrice / creditPerMonth;
  ...
  for (let i = 1; i <= creditPerMonth; i++) {
    ...
    dueDateDetails.push({
      ...
      pricePerMonth: Math.round(creditPrice),
    });
  }
  ...
  if (creditPerMonth > 1) {
    console.log('Due Dates Details :');
    console.log(dueDateDetails);
    // dueDates.forEach((dueDate) => {
    //   console.log(`${dueDate} - Price: ${Math.round(creditPrice)}`);
    // });
  } else if (creditPerMonth === 1) console.log(`Due Dates Details : ${dueDates[0]} - Price: ${Math.round(creditPrice)}`);
  const totalCredits = [];
  let totalCredit = 0;

  for (let i = 0; i < dueDates.length; i++) {
    totalCredit += creditPrice;
    totalCredits.push(Math.round(totalCredit));
  }

  //console.log(totalCredits);
  if (creditPerMonth > 0) console.log(`Total Price ${creditPerMonth} month : ${totalCredits.slice(-1)}`); //${Math.round(creditPrice * dueDates.length)}
